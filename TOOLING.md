...
- Reusable GitHub Actions workflows are called directly from the public
  ...
  promoted as `v1`; Renovate manages later digest updates as pull requests.
- Shell scripts and hook templates under `scripts/` are vendored from
  `digitalnsw/nswds-devops` at commit `f25f3a4`. They are refreshed manually
...
- `scripts/branch-name-config.sh` has a small personal override: `feature/` and
  ...
  prefixes.
- `scripts/openai-config.sh` defaults to `openai/gpt-5.4-mini`, a model verified
  against this account's AI Gateway routing. Set `OPENAI_MODEL` to override it.
